function [Error, AutoDataProcessParameter]=DPARSFA_serial(AutoDataProcessParameter,WorkingDir,SubjectListFile,IsAllowGUI)
...
% Modified by YAN Chao-Gan 091111. 1. Use different Affine Regularisation in Segmentation: East Asian brains (eastern) or European brains (mni). 2. Added a checkbox for removing first time points. 3.Added popup menu to delete selected subject by right click. 4. Close wait bar when program completed.
...
% Modified by Jing Wang, wangjing@xynu.edu.cn, 2024/09/11
...
% addpath([ProgramPath,filesep,'Subfunctions']);  % replaced
...
if (AutoDataProcessParameter.IsNeedConvertFunDCM2IMG==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            % InputFilename = repa_unique_slash(InputFilename);

            %YAN Chao-Gan 120817.
            y_Call_dcm2nii(InputFilename, OutputDir, 'DefaultINI');

            fprintf(['Converting Functional Images: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsNeedConvertT1DCM2IMG==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf(['Converting T1 Images: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
    end
    % fprintf('\n');
end
        ...
        for iFieldMapMeasure=1:length(FieldMapMeasures)
            if exist([AutoDataProcessParameter.DataProcessDir,filesep,'FieldMap',filesep,FieldMapMeasures{iFieldMapMeasure},'Raw'])
                ...
                for i=1:AutoDataProcessParameter.SubjectNum
                    ...
                    % InputFilename = repa_unique_slash(InputFilename);
                    y_Call_dcm2nii(InputFilename, OutputDir, 'DefaultINI');
                    fprintf(['Converting FieldMap ',FieldMapMeasures{iFieldMapMeasure},' Images: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
                end
                % fprintf('\n');
            end
        end
...
if (AutoDataProcessParameter.IsNeedReorientCropT1Img==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf(['Reorienting and Cropping Images: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
    end
    % fprintf('\n');
end
...
if (AutoDataProcessParameter.IsApplyDownloadedReorientMats==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf('Apply Downloaded Reorient Mats to functional images for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
    end
    ...
    if ~exist('UseNoCoT1Image','var')
        ...
        if isempty(DirCo)
            ...
            if length(DirImg)==1
                ...
                if 1
                    fprintf('No co* T1 image is found. Use the T1 image without co* instead.\n')
                    UseNoCoT1Image=1;
                else
                    return;
                end
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf('Apply Downloaded Reorient Mats to T1 images for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
...
if (AutoDataProcessParameter.RemoveFirstTimePoints>0)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            cd('..');
            fprintf(['Removing First ',num2str(AutoDataProcessParameter.RemoveFirstTimePoints),' Time Points: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsSliceTiming==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            fprintf(['Slice Timing Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done. ']);
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'A',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Slice Timing Corrected Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
        end
        % % fprintf('\n');
...
if isfield(AutoDataProcessParameter,'FieldMap')
    if (AutoDataProcessParameter.FieldMap.IsCalculateVDM==1)
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            fprintf(['Calculate VDM for FieldMap Correction Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
            spm_jobman('run',SPMJOB.matlabbatch);
        ...
        %Move the VDM files
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'FieldMap',filesep,'VDMImg',filesep,AutoDataProcessParameter.SubjectID{i}])
            if strcmpi(AutoDataProcessParameter.FieldMap.DataFormat,'PhaseDiffMagnitude')
...
if (AutoDataProcessParameter.IsRealign==1)
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf(['Realign Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done']);
        spm_jobman('run',SPMJOB.matlabbatch);
    end

    %Copy the Realign Parameters
    repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter']);
    if ~isempty(dir('*.ps'))
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'RealignParameter',filesep,AutoDataProcessParameter.SubjectID{i}]);
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            cd([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,filesep,AutoDataProcessParameter.SubjectID{i}]);
            repa_mkdir(['..',filesep,'..',filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'R',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            cd('..');
            fprintf(['Moving Head Motion Corrected Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n']);
        end
    end
    % fprintf('\n');
...
if (AutoDataProcessParameter.IsCalVoxelSpecificHeadMotion==1)
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
            ...
            %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'MeanVoxelSpecificHeadMotion_MeanTDvox']);
            % end
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'MeanVoxelSpecificHeadMotion_MeanTDvox']);
            ...
            %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'MeanVoxelSpecificHeadMotion_MeanFDvox']);
            % end
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'MeanVoxelSpecificHeadMotion_MeanFDvox']);
            ...
            fprintf(['\nGenerate voxel specific head motion: ',AutoDataProcessParameter.SubjectID{i},' ',FunSessionPrefixSet{iFunSession},' - Done\n\n.\n']);
...
if (AutoDataProcessParameter.IsNeedReorientT1ImgInteractively==1) && (7==exist([AutoDataProcessParameter.DataProcessDir,filesep,'T1Img'],'dir')) && AutoDataProcessParameter.IsAllowGUI
    ...
    %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'ReorientMats']);
    % end
    repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'ReorientMats']);
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf('Reorienting T1 Image Interactively for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
    end
    

    %Write the QC information as {WorkDir}/QC/RawT1ImgQC.tsv
    repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'QC'])
...
if (AutoDataProcessParameter.IsNeedReorientFunImgInteractively==1) && AutoDataProcessParameter.IsAllowGUI
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        clear global y_spm_image_Parameters
        fprintf('Reorienting Functional Images Interactively for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
    end
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf('Apply Reorient Mats to functional images for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
    end   
end
if ~isempty(Error)
    disp(Error);
...
if (AutoDataProcessParameter.IsBet==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum %YAN Chao-Gan, 180824. Cut for becuase of losing some processes. %for
    ...
    if (7==exist([AutoDataProcessParameter.DataProcessDir,filesep,'T1Img'],'dir')) %YAN Chao-Gan, 141101.
        ...
        for i=1:AutoDataProcessParameter.SubjectNum %YAN Chao-Gan, 180824. Cut for becuase of losing some processes. %for
            ...
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgBet',filesep,AutoDataProcessParameter.SubjectID{i}]);
...
if (AutoDataProcessParameter.IsNeedT1CoregisterToFun==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgCoreg',filesep,AutoDataProcessParameter.SubjectID{i}])
        ...
        fprintf(['Copying T1 image Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
    end
    % fprintf('\n');
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf(['Coregister Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done']);
        spm_jobman('run',SPMJOB.matlabbatch);
    end
end
...
if (AutoDataProcessParameter.IsNeedReorientInteractivelyAfterCoreg==1) && AutoDataProcessParameter.IsAllowGUI
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        clear global y_spm_image_Parameters
        fprintf('Reorienting Interactively After Coregistration for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
        cd('..');
    end
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf('Apply Reorient Mats (after T1-Fun coregistration) to functional images for %s: - Done\n\n',AutoDataProcessParameter.SubjectID{i});
    end
...
if (AutoDataProcessParameter.IsAutoMask==1)
    fprintf('\n\nAutoMask Generation Setup: %s - Done\n', AutoDataProcessParameter.SubjectID{i});
    folder_name = [AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'AutoMasks'];
    % if exist(folder_name,'dir')~=7
    %     repa_mkdir(folder_name);
    % end
    repa_mkdir(folder_name);
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
            ...
            % if HasDocker == 0
            %     CommandInit=sprintf('winpty docker run -i --rm -v %s:/opt/freesurfer/license.txt -v %s:/data -e SUBJECTS_DIR=/data/freesurfer cgyan/dpabi', fullfile(DPABIPath, 'DPABISurf', 'FreeSurferLicense', 'license.txt'), AutoDataProcessParameter.DataProcessDir);
        ...
        fprintf('\nGenerating AutoMasks for %s: - Done\n',AutoDataProcessParameter.SubjectID{i});
    end

    fprintf('Generating AutoMasks completed.\n\n');
end
...
if (AutoDataProcessParameter.IsSegment>=1)
    ...
    if isempty(DirT1ImgCoreg)
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            cd([AutoDataProcessParameter.DataProcessDir,filesep,'T1Img',filesep,AutoDataProcessParameter.SubjectID{i}]);
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,T1ImgSegmentDirectoryName,filesep,AutoDataProcessParameter.SubjectID{i}])
            % Check in co* image exist. Added by YAN Chao-Gan 100510.
            if UseNoCoT1Image==0
                DirImg=dir('c*.img');
                if ~isempty(DirImg)  %YAN Chao-Gan, 111114
            ...
            fprintf(['Copying T1 image Files from "T1Img" to',T1ImgSegmentDirectoryName,': ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
        end
        % fprintf('\n');
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            cd('..');
            fprintf(['Copying coregistered T1 image Files from "T1ImgCoreg": ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
        end
        % fprintf('\n');
    end
...
%             end
            ...
            fprintf(['\n\n\nSegment Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done']);
...
%             end
            ...
            fprintf(['Segment Setup: ',AutoDataProcessParameter.SubjectID{i},' - Done']);
...
if AutoDataProcessParameter.IsSegment==1
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        fprintf(['Normalize-Write T1 Image: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
...
if ((AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterRealign')))
    ...
    if ~isempty(T1ImgSegmentDirectoryName)
        if ~(2==exist([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'SegmentationMasks',filesep,'FunSpace_ThrdMask_',AutoDataProcessParameter.SubjectID{1},'_WM.nii'],'file'))
            ...
            %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'SegmentationMasks']);
            % end
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'SegmentationMasks']);
...
if (AutoDataProcessParameter.IsWarpMasksIntoIndividualSpace==1) || ((AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterRealign')))
    if ~(2==exist([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'WarpedMasks',filesep,AutoDataProcessParameter.SubjectID{1},'_','BrainMask_05_91x109x91','.nii'],'file'))
        ...
        %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'WarpedMasks']);
        % end
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks',filesep,'WarpedMasks']);
        ...
        if (7==exist([AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgNewSegment',filesep,AutoDataProcessParameter.SubjectID{1}],'dir')) 
            ...
            for i=1:AutoDataProcessParameter.SubjectNum
                ...
                %     fprintf('Warp Masks (%s) for "%s" to individual space using DARTEL flow field (in T1ImgNewSegment) genereated by DARTEL.\n',AMaskFilename, AutoDataProcessParameter.SubjectID{i});
                ...
                fprintf('\n');
                OutFile=[];
                fprintf('Warp Masks for %s to individual space using DARTEL flow field (in T1ImgNewSegment) genereated by DARTEL.\n', AutoDataProcessParameter.SubjectID{i});
                for iMask=1:length(MasksName)
                    AMaskFilename = MasksName{iMask};
                    fprintf('%s\n', AMaskFilename);
                    [pathstr, name, ext] = fileparts(AMaskFilename);
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            if (7==exist([AutoDataProcessParameter.DataProcessDir,filesep,'T1ImgNewSegment',filesep,AutoDataProcessParameter.SubjectID{1}],'dir'))
                ...
                for iROI=1:length(NeedWarpMaskNameSet)
                    fprintf('Warp %s Mask (%s) for "%s" to individual space using DARTEL flow field (in T1ImgNewSegment) genereated by DARTEL.\n',Suffix,NeedWarpMaskNameSet{iROI}, AutoDataProcessParameter.SubjectID{i});
                end
                fprintf('\n');
...
if (AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterRealign'))
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            if ~isempty(CompCorMasks)
                ...
                %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'Covs']);
                % end
                repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'Covs']);
            ...
            % fprintf('\nRegressing out covariates for subject %s %s',AutoDataProcessParameter.SubjectID{i},FunSessionPrefixSet{iFunSession});
    ...
    %Copy the Covariates files to DataProcessDir\{AutoDataProcessParameter.StartingDirName}+C
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'C',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Covariates Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsFilter==1) && (strcmpi(AutoDataProcessParameter.Filter.Timing,'BeforeNormalize'))
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'F',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Filtered Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsNormalize>0) && strcmpi(AutoDataProcessParameter.Normalize.Timing,'OnFunctionalData')
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        if (AutoDataProcessParameter.IsNormalize==1) %Normalization by using the EPI template directly
            ...
            fprintf(['Normalize: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        ...
        if (AutoDataProcessParameter.IsNormalize==2) %Normalization by using the T1 image segment information
            ...
            fprintf(['Normalize-Write: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        ...
        if (AutoDataProcessParameter.IsNormalize==3) %Normalization by using DARTEL %YAN Chao-Gan, 111111.
            ...
            spm_jobman('run',SPMJOB.matlabbatch);
            fprintf(['Normalization by using DARTEL: ',AutoDataProcessParameter.SubjectID{i},' - Done\n']);
        end
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'W',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Normalized Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
        end
        % fprintf('\n');
    end
...
%     end
    ...
    if AutoDataProcessParameter.IsAllowGUI %YAN Chao-Gan, 161011. Generate the pictures only if GUI is allowed.
        ...
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'PicturesForChkNormalization']);
        ...
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            fprintf(['Generating the pictures for checking normalization: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n\n\n']);
...
if (AutoDataProcessParameter.IsSmooth>=1) && strcmpi(AutoDataProcessParameter.Smooth.Timing,'OnFunctionalData')
    if (AutoDataProcessParameter.IsSmooth==1)
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            fprintf(['Smooth: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'S',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Smoothed Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsDetrend==1)
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'D',filesep,AutoDataProcessParameter.SubjectID{i}])
            ...
            fprintf(['Moving Detrended Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
...
if (AutoDataProcessParameter.IsWarpMasksIntoIndividualSpace==0) && ((AutoDataProcessParameter.IsCalALFF==1)||( (AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterNormalize')) )||(AutoDataProcessParameter.IsCalReHo==1)||(AutoDataProcessParameter.IsCalDegreeCentrality==1)||(AutoDataProcessParameter.IsCalFC==1)||(AutoDataProcessParameter.IsCalVMHC==1)||(AutoDataProcessParameter.IsCWAS==1))
    ...
    %     repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks']);
    % end
    repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'Masks']);
    ...
    fprintf('Resampling masks...\n');
    fprintf('Resampling the following Masks to the resolution of functional images:\n');
    for iMask=1:length(MasksName)
        AMaskFilename = MasksName{iMask};
        fprintf('%s\n',AMaskFilename);
...
if (AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterNormalize')) %if (AutoDataProcessParameter.IsCovremove==1) && (strcmpi(AutoDataProcessParameter.Covremove.Timing,'AfterNormalizeFiltering'))
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            rmdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,filesep,AutoDataProcessParameter.SubjectID{i}, '_Covremoved']);
            fprintf(['Moving Coviables Removed Files: ',AutoDataProcessParameter.SubjectID{i},' - Done\n\n']);
        end
        % fprintf('\n');
    end
    AutoDataProcessParameter.StartingDirName=[AutoDataProcessParameter.StartingDirName,'C']; %Now StartingDirName is with new suffix 'C'
...
if (AutoDataProcessParameter.IsCalALFF==1)
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'ALFF_',AutoDataProcessParameter.StartingDirName]);
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'fALFF_',AutoDataProcessParameter.StartingDirName]);
...
if (AutoDataProcessParameter.IsScrubbing==1) && (strcmpi(AutoDataProcessParameter.Scrubbing.Timing,'AfterPreprocessing'))
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            ...
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'B',filesep,AutoDataProcessParameter.SubjectID{i}]);
...
if (AutoDataProcessParameter.IsCalReHo==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'ReHo_',AutoDataProcessParameter.StartingDirName]);
...
if (AutoDataProcessParameter.IsCalDegreeCentrality==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'DegreeCentrality_',AutoDataProcessParameter.StartingDirName]);
...
if (~isempty(AutoDataProcessParameter.CalFC.ROIDef)) || (AutoDataProcessParameter.IsDefineROIInteractively==1)
    ...
    for i=1:AutoDataProcessParameter.SubjectNum
        ...
        if AutoDataProcessParameter.IsWarpMasksIntoIndividualSpace==1
            ...
            % fprintf('Reslice the following %sMasks for "%s" since the dimension of mask mismatched the dimension of the functional data.\n',Suffix, AutoDataProcessParameter.SubjectID{i});
            % Check if the ROI mask is appropriate
            for iROI=1:length(SubjectROI)
                AMaskFilename=SubjectROI{iROI};
                if exist(SubjectROI{iROI},'file')==2
...
if (AutoDataProcessParameter.IsCalFC==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'FC_',AutoDataProcessParameter.StartingDirName]);
...
if (AutoDataProcessParameter.IsExtractROISignals==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        ...
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'ROISignals_',AutoDataProcessParameter.StartingDirName]);
...
if (AutoDataProcessParameter.IsCWAS==1)
    for iFunSession=1:1
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'CWAS_',AutoDataProcessParameter.StartingDirName]);
...
if AutoDataProcessParameter.IsNormalizeToSymmetricGroupT1Mean==1
    ...
    repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,'SymmetricGroupT1MeanTemplate']);
    ...
    %Normalize to symmetric template
    fprintf('Normalize to symmetric template...');
    for i=1:AutoDataProcessParameter.SubjectNum
        FileList=[];
        for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
            cd([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,filesep,AutoDataProcessParameter.SubjectID{i}]);
        ...
        fprintf(['Normalize to symmetric group T1 mean Template: ',AutoDataProcessParameter.SubjectID{i},' - Done\n']);
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for i=1:AutoDataProcessParameter.SubjectNum
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'sym',filesep,AutoDataProcessParameter.SubjectID{i}])
...
if (AutoDataProcessParameter.IsCalVMHC==1)
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},'Results',filesep,'VMHC_',AutoDataProcessParameter.StartingDirName]);
...
if (AutoDataProcessParameter.IsNormalize>0) && strcmpi(AutoDataProcessParameter.Normalize.Timing,'OnResults')
    ...
    for iFunSession=1:AutoDataProcessParameter.FunctionalSessionNumber
        for iMeasure=1:length(MeasureSet)
            repa_mkdir([AutoDataProcessParameter.DataProcessDir,filesep,FunSessionPrefixSet{iFunSession},AutoDataProcessParameter.StartingDirName,'W',filesep,MeasureSet{iMeasure}])
            ...
            fprintf(['Moving Normalized Files: ',MeasureSet{iMeasure},' - Done\n\n']);
        end
        % fprintf('\n');
    end
    AutoDataProcessParameter.StartingDirName=[AutoDataProcessParameter.StartingDirName,'W']; %Now StartingDirName is with new suffix 'W'
...
% cd to the starting folder that contains preprocess_DPARSFA*.m
cd(AutoDataProcessParameter.DataProcessDir);
cd('..');

% fprintf('%s\n',repmat('-',1,72));
% fprintf('Congratulations! The DPARSFA processing has been successfully completed.\n');
% fprintf('%s\n',repmat('-',1,72));