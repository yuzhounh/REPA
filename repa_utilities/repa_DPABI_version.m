function [original_ver, converted_version, converted_ver] = repa_DPABI_version()
    % Extract DPABI version from MATLAB path
    original_ver = original_dpabi_version();
    
    % Convert the version to the new format
    [converted_version, converted_ver] = converted_dpabi_version(original_ver);
end
...
function [version, ver] = converted_dpabi_version(version_string)
